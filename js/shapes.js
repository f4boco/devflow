// Renderização de Todos os Símbolos Hand-Drawn da Biblioteca
const ShapeRenderer = {
  draw(rc, ctx, shape, elements = []) {
    ...
    if (type === 'line' || type === 'arrow') {
      ...
      if (startShape || endShape) {
        ...
        const p1 = startShape ? this.getEdgeIntersection(startCenter, endCenter, startShape) : startCenter;
        const p2 = endShape ? this.getEdgeIntersection(endCenter, startCenter, endShape) : endCenter;
    ...
    if (shape.selected) {
      this.drawHandles(ctx, { x, y, width: w, height: h, type });
    }
  },
